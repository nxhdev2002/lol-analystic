"""
OpenAI-compatible AI Integration Module
Handles all communications with OpenAI-compatible API for AI analysis.
Supports OpenAI API and any OpenAI-compatible endpoints (e.g., local LLMs).
"""
...
class OpenAIAI:
    """Class to interact with OpenAI-compatible API for AI analysis."""
    ...
    def __init__(self, api_key, base_url="https://api.openai.com/v1", model="gpt-4o-mini"):
        ...
        Args:
            ...
            base_url (str): Base URL for OpenAI API (default: https://api.openai.com/v1)
    ...
    def initialize(self):
        """
        Initialize OpenAI client (placeholder for API calls).
    ...
    def analyze_lol_matches(self, match_data, player_name):
        """
        Send match data to OpenAI and get AI analysis.
        ...
        try:
            ...
            payload = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "Mày là một thằng toxic chuyên phân tích game League of Legends. Mày hay chửi bậy, mắng người ta ngu, troll không thương tiếc nhưng vẫn đưa ra phân tích đúng. Giọng điệu của mày: cực kỳ toxic, hay chửi thề, mắng người ta đồ noob, đồ gà, đồ ngu, troll hết cỡ. Dùng các từ như 'đ*t mẹ', 'ngu vl', 'gà vãi', 'trash', 'noob', 'ăn hành', 'feed như chó', 'đội lỗ ăn mắm', 'cân não đéo có não'. Sử dụng emoji để làm nổi bật sự toxic 🔥💀🤡💩🗑️."
                    },
                    {
                        "role": "user",
                        "content": prompt
    ...
    def analyze_lol_rank(self, rank_data):
        """
        Send rank data to OpenAI and get AI analysis.
...
"""
    ...
    def _create_rank_prompt(self, game_name, tag_line, rank_summary):
        """
        Create the prompt for OpenAI for rank analysis.
        ...
        """
        prompt = f"""Bây giờ hãy phân tích xếp hạng hiện tại của thằng "{game_name}#{tag_line}".
...
"""
    ...
    def _create_prompt(self, player_name, matches_summary):
        ...
        Returns:
            str: The complete prompt
        """
        prompt = f"""Bây giờ hãy phân tích các trận đấu gần nhất của thằng "{player_name}".
...
Hãy viết phân tích toxic hết cỡ, chửi bậy thoải mái, mắng mỏ đồ ngu nhưng vẫn đưa ra phân tích chính xác. Sử dụng emoji để làm nổi bật sự toxic 🔥💀🤡💩🗑️."""
    ...
    def ask_lol_question(self, rank_data, match_data, player_display, question):
        """
        Send rank and match data with a custom question to OpenAI and get AI answer.
        ...
        try:
            ...
            payload = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "Mày là một thằng toxic chuyên phân tích game League of Legends. Mày hay chửi bậy, mắng người ta ngu, troll không thương tiếc nhưng vẫn đưa ra câu trả lời đúng. Giọng điệu của mày: cực kỳ toxic, hay chửi thề, mắng người ta đồ noob, đồ gà, đồ ngu, troll hết cỡ. Dùng các từ như 'đ*t mẹ', 'ngu vl', 'gà vãi', 'trash', 'noob', 'ăn hành', 'feed như chó', 'đội lỗ ăn mắm', 'cân não đéo có não'. Sử dụng emoji để làm nổi bật sự toxic 🔥💀🤡💩🗑️."
                    },
                    {
                        "role": "user",
                        "content": prompt
    ...
    def _create_ask_prompt(self, player_display, rank_summary, matches_summary, question):
        """
        Create the prompt for OpenAI with a custom question.
        ...
        Returns:
            str: The complete prompt
        """
        prompt = f"""Bây giờ hãy trả lời câu hỏi về thằng "{player_display}".
